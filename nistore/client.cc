...
Client::Client(Proto mode, string configPath, int nShards, std::string host,
               const Security &s)
    : transport(0.0, 0.0, 0) {
  ...
  ReplicaAddress addr(host, "");
  tss = new specpaxos::vr::VRClient(tssConfig, addr, &transport);
  /*
  switch (mode) {
      case PROTO_VR:
      ...
      case PROTO_SPEC:
          ...
          break;
      case PROTO_FAST:
          tss = new specpaxos::fastpaxos::FastPaxosClient(tssConfig,
  &transport); break; default: NOT_REACHABLE();
  }*/
  ...
  for (int i = 0; i < nShards; i++) {
    ...
    switch (mode) {
      case PROTO_VR:
        shard[i] = new specpaxos::vr::VRClient(shardConfig, addr, &transport);
        break;
      case PROTO_SPEC:
        shard[i] =
            new specpaxos::spec::SpecClient(shardConfig, addr, &transport);
        break;
      case PROTO_FAST:
        shard[i] = new specpaxos::fastpaxos::FastPaxosClient(shardConfig, addr,
                                                             &transport);
        break;
      case PROTO_PBFT:
        shard[i] =
            new specpaxos::pbft::PbftClient(shardConfig, addr, &transport, s);
        break;
      case PROTO_MINBFT:
        shard[i] = new specpaxos::minbft::MinBFTClient(shardConfig, addr,
                                                       &transport, s);
        break;
      case PROTO_TOMBFT:
        shard[i] = new specpaxos::tombft::TomBFTClient(shardConfig, addr,
                                                       &transport, s);
        break;
      default:
        NOT_REACHABLE();
...
void Client::send_begin(unsigned int i) {
  ...
  transport.Timer(0, [=]() {
    shard[i]->Invoke(request_str, bind(&Client::beginCallback, this, i,
...
bool Client::Get(const string &key, string &value) {
  ...
  transport.Timer(0, [=]() {
    shard[i]->Invoke(request_str, bind(&Client::getCallback, this, i,
                                       placeholders::_1, placeholders::_2));
  });
...
void Client::Put(const string &key, const string &value) {
  ...
  transport.Timer(0, [=]() {
    shard[i]->Invoke(request_str, bind(&Client::putCallback, this, i,
                                       placeholders::_1, placeholders::_2));
  });
...
bool Client::Commit() {
    ...
    for (set<int>::iterator it = all_participants.begin();
      ...
      transport.Timer(0, [=]() {
        shard[*it]->Invoke(request_str,
                           bind(&Client::prepareCallback, this, *it,
                                placeholders::_1, placeholders::_2));
      });
    }
  ...
  if (nreplies == all_participants.size() && status == true) {
    // Get a timestamp from the tss
    {
      ...
      transport.Timer(0, [=]() {
        tss->Invoke("", bind(&Client::tssCallback, this, placeholders::_1,
                             placeholders::_2));
      });

      Debug("Waiting for TSS reply");
      cv.wait(lk);
      Debug("TSS reply received \"%s\"", replica_reply.c_str());
    }
    ...
    // Send Commit to all shards
    {
      ...
      for (set<int>::iterator it = all_participants.begin();
        ...
        transport.Timer(0, [=]() {
          shard[*it]->Invoke(request_str,
                             bind(&Client::commitCallback, this, *it,
                                  placeholders::_1, placeholders::_2));
        });
      }
...
void Client::Abort() {
  ...
  for (set<int>::iterator it = all_participants.begin();
    ...
    if (yes_participants.find(*it) != yes_participants.end()) {
      ...
      transport.Timer(0, [=]() {
        shard[*it]->Invoke(request_str,
                           bind(&Client::abortCallback, this, *it,
                                placeholders::_1, placeholders::_2));
      });
    }
  }

  if (aborts_sent == 0) return;
...
void Client::beginCallback(const int index, const string &request_str,
...
void Client::getCallback(const int index, const string &request_str,
                         const string &reply_str) {
  lock_guard<mutex> lock(cv_m);
...
void Client::putCallback(const int index, const string &request_str,
                         const string &reply_str) {
  lock_guard<mutex> lock(cv_m);
...
void Client::prepareCallback(const int index, const string &request_str,
                             const string &reply_str) {
  lock_guard<mutex> lock(cv_m);
...
void Client::commitCallback(const int index, const string &request_str,
                            const string &reply_str) {
  lock_guard<mutex> lock(cv_m);
...
void Client::abortCallback(const int index, const string &request_str,
                           const string &reply_str) {
  lock_guard<mutex> lock(cv_m);
