...
Client::Client(
    Proto mode, string configPath, int nShards, const ReplicaAddress &addr)
    : transport(0.0, 0.0, 0) {
    ...
    if (tssConfigStream.fail()) {
        fprintf(
            stderr, "unable to read configuration file: %s\n",
            tssConfigPath.c_str());
        exit(0);
    }
    ...
    for (int i = 0; i < nShards; i++) {
        ...
        case PROTO_VR:
            shard[i] =
                new specpaxos::vr::VRClient(shardConfig, addr, &transport);
            break;
        case PROTO_SPEC:
            shard[i] =
...
void Client::send_begin(unsigned int i) {
    ...
    transport.Timer(0, [=]() {
        shard[i]->Invoke(
            request_str, bind(
                             &Client::beginCallback, this, i, placeholders::_1,
                             placeholders::_2));
    });
}
...
bool Client::Get(const string &key, string &value) {
    ...
    transport.Timer(0, [=]() {
        shard[i]->Invoke(
            request_str, bind(
                             &Client::getCallback, this, i, placeholders::_1,
                             placeholders::_2));
    });
...
void Client::Put(const string &key, const string &value) {
    ...
    transport.Timer(0, [=]() {
        shard[i]->Invoke(
            request_str, bind(
                             &Client::putCallback, this, i, placeholders::_1,
                             placeholders::_2));
    });
    ...
    // 1. Send commit-prepare to all shards.
    {
        ...
        for (set<int>::iterator it = all_participants.begin();
            ...
            transport.Timer(0, [=]() {
                shard[*it]->Invoke(
                    request_str, bind(
                                     &Client::prepareCallback, this, *it,
    ...
    if (nreplies == all_participants.size() && status == true) {
        // Get a timestamp from the tss
        {
            ...
            transport.Timer(0, [=]() {
                tss->Invoke(
                    "", bind(
                            &Client::tssCallback, this, placeholders::_1,
                            placeholders::_2));
            });
...
void Client::beginCallback(
    const int index, const string &request_str, const string &reply_str) {
