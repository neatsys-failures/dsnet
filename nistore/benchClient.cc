...
int main(int argc, char **argv) {
    ...
    nistore::Client client(
        mode, configPath, nShards, ReplicaAddress(host, "0"));
    ...
    while (1) {
        ...
        beginCount++;
        beginLatency +=
            ((t4.tv_sec - t1.tv_sec) * 1000000 + (t4.tv_usec - t1.tv_usec));
        ...
        fprintf(
            stderr, "%d %ld.%06ld %ld.%06ld %ld %d\n", nTransactions + 1,
            t1.tv_sec, (long)t1.tv_usec, t2.tv_sec, (long)t2.tv_usec, latency,
            status ? 1 : 0);
