...
int main(int argc, char **argv) {
  ...
  int opt;
  while ((opt = getopt(argc, argv, "c:d:h:N:l:w:k:f:m:e:s:")) != -1) {
    switch (opt) {
  ...
  dsnet::HomogeneousSecurity security(seq_signer, seq_verifier, seq_signer,
                                      seq_verifier);
  nistore::Client client(mode, configPath, nShards, host, security);
  ...
  while (1) {
    ...
    // t1.tv_sec,
    //         (long)t1.tv_usec, t2.tv_sec, (long)t2.tv_usec, latency,
    //         status?1:0);
